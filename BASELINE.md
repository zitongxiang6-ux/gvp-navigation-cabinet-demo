...
- 对应 PRD：`../../requirements/prd/REQ-2026-001-004-导航与配电箱增强-PRD-V1.11-20260828.docx`
...
4. 左侧当前只显示 `#device-functions`、`#history`、`#devices`；`#overview` 和 `#navigation` 的入口暂时隐藏，但路由、页面与实现代码必须保留。旧 `#cabinet-devices` 地址兼容跳转到 `#device-functions`，不显示独立菜单。
...
默认地址：`http://127.0.0.1:4173/?v=20260828-device-function-sort-cabinet-order#device-functions`

## 边界

- 这是脱敏 Mock 数据驱动的评审 Demo，不连接生产接口。
- Demo 展示产品要求和评审假设，不证明功能已经开发、测试、验收或发布。
- 2026-08-28 用户最新确认：以现有“设备功能”页面为唯一合并基线，原有三个筛选、七个操作按钮、复选框、全部原字段和分页均保留，只新增配电箱平铺分组、“未分类”、在线数/总数、联动筛选和只读“配电箱”字段；功能名称、设备备注、设备名称保留上下排序入口，“配电箱”位于“所属页面”右侧。该结论已同步进入 PRD V1.11，Demo 与 PRD 均为评审产物，不代表已发布。
