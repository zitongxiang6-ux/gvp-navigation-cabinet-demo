...
默认地址：`http://127.0.0.1:4173/?v=20260831-cabinet-function-query#cabinet-functions`

## 边界

- 这是脱敏 Mock 数据驱动的评审 Demo，不连接生产接口。
- Demo 展示产品要求和评审假设，不证明功能已经开发、测试、验收或发布。
- 2026-08-31 用户最新确认：恢复独立【配电箱功能查询】，将此前加入【设备功能】的配电箱侧栏、筛选和字段拆出；【设备功能】原有筛选、七个操作按钮、复选框、原字段、排序和分页保持不变。独立查询页左侧为区域树和所选区域下的现有配电箱/未分类，右侧只读展示设备功能明细、状态汇总与故障，不提供其他操作。该要求已同步进入需求详情，PRD V1.11 仍是前一版评审口径；Demo 与 PRD 均不代表已发布。
