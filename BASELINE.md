...
默认地址：`http://127.0.0.1:4173/?v=20260831-remove-device-functions#cabinet-functions`

## 边界

- 这是脱敏 Mock 数据驱动的评审 Demo，不连接生产接口。
- Demo 展示产品要求和评审假设，不证明功能已经开发、测试、验收或发布。
- 2026-08-31 用户最新确认：删除【设备功能】整个页面，包括主导航菜单、页面路由、渲染交互与专属样式；旧地址自动进入【配电箱功能查询】。独立查询页继续使用收窄且左右并排的区域树与配电箱/未分类筛选，右侧直接展示设备功能明细表格，不显示标题、筛选说明、只读标记、状态汇总和故障记录，也不提供其他操作。该要求已同步进入需求详情，PRD V1.11 仍是前一版评审口径；Demo 与 PRD 均不代表已发布。
